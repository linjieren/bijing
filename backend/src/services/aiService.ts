...
function sanitizeContent(text: string): string {
  ...
  // 移除 AI 可能误输出的结构标记（起承转合等）
  text = text.replace(/^[起承转合][：:]\s*/gm, '');
  // 清理连续的空行
  text = text.replace(/\n{3,}/g, '\n\n');
  return text.trim();
}
...
async function callKimi(messages: KimiMessage[], temperature = 0.8, maxTokens = 8000): Promise<string> {
  if (!KIMI_API_KEY) {
